...
std: [Location]
	...
	lon: 经度
*/
func Location_Nc(lat, lon, title, content string) Segment {
	return Segment{
		Type: TYPE_LOCATION,
		Data: map[string]any{
			"lat":     lat,
...
std: [Music]
	...
	image: 图片
*/
func Music_Nc(typ, url, audio, title, content, image string) Segment {
	return Segment{
		Type: TYPE_MUSIC,
		Data: map[string]any{
			"type":    typ,
...
*/
func Poke_Nc(typ, id string, strength ...string) Segment {
	if len(strength) == 0 {
		return Segment{
			Type: TYPE_POKE,
			Data: map[string]any{
...
std: [At]
	...
	name(已废弃): 显示的文本
*/
func At_Nc[numT NumberT](userId numT, name string) Segment {
	return Segment{
		Type: TYPE_AT,
		Data: map[string]any{
			"user_id": userId,
...
std: [Face]
	...
	large(可选): 是否大表情
*/
func Face_Nc[numT NumberT](id numT, large ...bool) Segment {
	if len(large) == 0 {
		return Segment{
			Type: TYPE_FACE,
			Data: map[string]any{
...
std: [Image]
	...
	subType: 图片子类型
*/
func Image_Nc[fileT FileT](file fileT, filename, url, summary, subType string) Segment {
	return Segment{
		Type: TYPE_IMAGE,
		Data: map[string]any{
			"file":     file,
